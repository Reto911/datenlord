    ...
    async fn rename_exchange_helper(
        ...
        loop {
            ...
            if let Some(mut new_parent_node) = new_parent_node {
                ...
                let mut old_entry = old_parent_node.get_dir_data_mut().remove(old_name).unwrap_or_else(||unreachable!("Impossible case when exchange,\
                                                                                                                                    as {old_name} under {old_parent} is checked to be existed."));
                let mut new_entry = new_parent_node.get_dir_data_mut().remove(new_name).unwrap_or_else(||unreachable!("Impossible case when exchange,\
                                                                                                                                    as {new_name} under {new_parent} is checked to be existed."));
                old_entry.set_entry_name(new_name.into());
                new_entry.set_entry_name(old_name.into());
                old_parent_node.insert_entry_for_rename(new_entry);
                new_parent_node.insert_entry_for_rename(old_entry);
                ...
                let mut old_entry = old_parent_node.get_dir_data_mut().remove(old_name).unwrap_or_else(||unreachable!("Impossible case when exchange,\
                                                                                                                                    as {old_name} under {old_parent} is checked to be existed."));
                let mut new_entry = old_parent_node.get_dir_data_mut().remove(new_name).unwrap_or_else(||unreachable!("Impossible case when exchange,\
                                                                                                                                    as {new_name} under {new_parent} is checked to be existed."));

                old_entry.set_entry_name(new_name.into());
                new_entry.set_entry_name(old_name.into());
                old_parent_node.insert_entry_for_rename(new_entry);
                old_parent_node.insert_entry_for_rename(old_entry);
            ...
            debug!("exchange() found transaction commit failed when exchange, retrying...");
        }
    }
...
impl<S: S3BackEnd + Send + Sync + 'static> S3MetaData<S> {
    ...
    async fn exchange_pre_check<T: MetaTxn + ?Sized>(
        ...
        let old_parent_node = txn
            ...
            .await?
            // return ENOENT instead of InconsistentFS, because this function may run multiple times
            // when the transaction commit fails for that some relative metadata changed during the exchange
            .ok_or_else(|| {
