...
impl Storage for BackendWrapper {
    ...
    async fn store(&self, ino: INum, block_id: usize, block: Block) {
        let path = Self::get_block_path(ino, block_id);
    ...
    async fn remove(&self, ino: INum) {
        self.operator
            .remove_all(&Self::get_file_path(ino))
            .await
    ...
    async fn truncate(&self, ino: INum, from_block: usize, to_block: usize, fill_start: usize) {
        let paths =
            stream::iter(to_block..from_block).map(|block_id| Self::get_block_path(ino, block_id));
        ...
        let file_exists = self
            ...
            .is_exist(&file_path)
            .await
            .unwrap_or_else(|e| panic!("Failed to check if file ino={ino} exists: {e}"));
        ...
        if file_exists {
            ...
            if to_block > 0 && fill_start < self.block_size {
                ...
                let path = Self::get_block_path(ino, truncate_block_id);
...
mod tests {
    ...
    use super::{BackendWrapper, Block, Storage};
    ...
    /// Prepare a backend
    fn prepare_backend(root: &str) -> BackendWrapper {
        let mut builder = Fs::default();
        builder.root(root);
        let op = Operator::new(builder).unwrap().finish();

        BackendWrapper::new(op, BLOCK_SIZE_IN_BYTES)
    }
